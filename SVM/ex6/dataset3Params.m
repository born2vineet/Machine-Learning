...
best = zeros(3,1); % [predAccuracy; tryC; tryS]
trialNum = 1;
...
for tryC = incrementC
    for tryS = incrementSigma
        ...
        % Training the model with tryC and TryS
        model = svmTrain(X, y, tryC, @(x1, x2) gaussianKernel(x1, x2, tryS));
        
        % Predicting the value on validation set
        prediction = svmPredict(model, Xval);
        
        % Calculating Prediction Accuracy and then selecting the best model
        predAccuracy = mean(double(prediction == yval));
        if(predAccuracy > best(1))
          fprintf(['Best so far %f\n'], predAccuracy);
          best = [predAccuracy, tryC, tryS];
        end
    end
end
